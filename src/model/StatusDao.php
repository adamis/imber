<?php
	...
	class StatusDao {
		 ...
		 * Constant variables
		 */
		private $create = "INSERT INTO status (
				status
				, cadastrado
				, modificado
				) VALUES";
		...
		public $read = 
				...
				, status.cadastrado AS 'status.cadastrado'
				, status.modificado AS 'status.modificado'
				";
				
		private $update = "UPDATE status SET";
		private $delete = "DELETE FROM status";
		
		public $from = "status status";
		 ...
		 * @param {Status}status
		 */
		public function setCreate($status) {		
		 ...
		 * @param {String} where
		 */
		public function setUpdate($status, $where) {
			$this->setWhere($where);
		...
		private function setSize($where) {
			...
			$result = $this->connection->execute(
					"SELECT count(1) AS 'status.size' from status" . $this->getWhere());

			while ($row = $result->fetch_assoc()) {		
				$this->setResponse(0, "status.size", $row["status.size"]);
				
				$pages = ceil($row["status.size"] / $this->connection->getItensPerPage());
				
				$this->setResponse(0, "status.pages", $pages);
				...
				$this->setResponse(0, "status.pagination", $pagination);
			}
		...
		public function read($where, $order, $wp) {
			...
			while ($row = $result->fetch_assoc()) {
				$this->setResponse($line, "status.id", $row["status.id"]);
				$this->setResponse($line, "status.status", $row["status.status"]);
				$this->setResponse($line, "status.cadastrado", modelDateTime($row["status.cadastrado"]));
				$this->setResponse($line, "status.modificado", modelDateTime($row["status.modificado"]));
			
				$this->setResponse($line, "status.line", $line);
		...
		public function update($status) {
			...
			$this->setUpdate($status, "status.id = " . $status->getId());
		...
		public function combo($selected, $order) {
			...
			while ($row = $result->fetch_assoc()) {
				$this->setResponse($size, "status.id", $row["status.id"]);
				$this->setResponse($size, "status.status", $row["status.status"]);
			
				if ($row["status.id"] == $selected)
					$this->setResponse($size, "status.selected", "selected");
				else
					$this->setResponse($size, "status.selected", "");
