<?php
	...
	if ($method == "stateCreate" || $method == "stateRead" || $method == "stateUpdate" ||
		...
		$_USER[0]["users"] = $daoFactory->getUsersDao()->read("users.id = " .
				($session[0]["users.id"] == null ? 0 : 
				$session[0]["users.id"]), "", true);
				
		if ($_USER[0]["users"][0]["users.photo"] == "")
			$_USER[0]["users"][0]["users.photo"] = "user.png";
			
		if ($session[0]["users.id"] != null)		
			$darth->setTheme($_USER[0]["users"][0]["themes.identifier"]);	
